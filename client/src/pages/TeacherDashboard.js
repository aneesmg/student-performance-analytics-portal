...
const TeacherDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const [perfStats, studentsRes] = await Promise.all([
          ...
          studentAPI.getAll({ limit: 4 }).catch(() => null),
        ]);
  ...
  const classStats = stats ? [
    { label: 'Total Students', value: `${stats.totalStudents || 0}`, icon: UsersIcon, color: '#2e7d32', change: '+4 this sem' },
    { label: 'Class Average', value: `${(stats.avgOverall || 0).toFixed(1)}%`, icon: GradeIcon, color: '#1565c0', change: '+3.2%' },
    { label: 'Pass Rate', value: '89%', icon: TrendingUpIcon, color: '#6a1b9a', change: '+1.5%' },
    { label: 'Avg Attendance', value: `${(stats.avgAttendance || 0).toFixed(1)}%`, icon: AttendanceIcon, color: '#e65100', change: '-2%' },
  ] : [
